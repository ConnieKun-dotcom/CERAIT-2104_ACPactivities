...
class PayrollSystem:
    ...
    def create_payslip(self):
        try:
            ...
            print_centered("=" * 130)
            print_centered(f"Employee ID: {emp.get_emp_id()}")
            print_centered(f"Name: {emp.get_name()}")
            print_centered(f"Job Title: {emp.get_job_title()}")
            print_centered(f"Phone Number: +63-{emp.get_phone()}")
            print_centered(f"Department: {emp.get_department()}")
            print_centered("-" * 130)
            ...
            try:
                ...
                if salary_in_advance < 0:
                    raise ValueError("Salary advance cannot be negative")
                    
                incentives = self.get_numeric_input("\t\t\t\tEnter Incentives: ")
                if incentives < 0:
                    raise ValueError("Incentives cannot be negative")
            ...
            try:
                ...
                # Calculate totals
                total_earnings = base_salary + incentives + bonus
                total_deductions = (salary_in_advance + sss_employee_contribution + 
    ...
    def display_payslip(self, emp_id):
        ...
        if emp_id not in self.__payslips or emp_id not in self.__employees:
            print_centered("Payslip not found.", Colors.RED)
            return
    ...
    def setup_predefined_employees(self):
        ...
        self.__employees = {
            ...
            "FIN-C-0005": ContractEmployee("FIN-C-0005", "Mary Merrier", "Financial Analyst", "mary.merrier@example.com", "09123456787", "Finance", "Robert Brown", "2013-06-20", "1988-4-11"),
            "ENG-C-0010": ContractEmployee("ENG-C-0010", "Lucas Nguyen", "Computer Engineer", "lucas.nguyen@example.com", "09123456786", "Engineering", "Aiden Kim", "2020-2-01", "1992-03-25"),
